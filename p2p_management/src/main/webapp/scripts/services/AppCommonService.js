...
option.url = {
	...
	'api_updateUser' : '/p2p_action/updateUser/',
	'api_jyjl' : '/p2p_action/jiaoYiJiLu/'
};
option.header = {
	'headers' : {
...
angular
					...
					return {
						...
						getUserList : function() {
							...
							return $http.post(option.url.api_user
									+ 'findAllUser', option.header);
							// /p2p_action/user/findAllUser
						},
						// 根据ID获得用户
						getUserById : function($str) {
							return $http.post(option.url.api_user
									+ 'getUserById', $str, option.header);
							// /p2p_action/product/findProductById
						},

						// 提交修改操作
						commitUser : function($str) {

							return $http.post(option.url.api_updateUser
									+ 'update', $str, option.header);
						},
						// 删除用户
						del : function($str) {
							return $http.post(option.url.api_user + 'del',
									$str, option.header);
						}
					}
				})
				...
				function($http) {
					return {
						...
						checkUserName : function($str) {
							...
							// 相当于$http.post('/p2p_action/user/validateUserName',
							// $str, option.header);
						},
						// 验证手机是否可用
		...
		.factory(
				...
				function($http) { // 将$http注入，就可以向服务器发送请求
					return {
						findPuerchase : function() {

							return $http.post(option.url.api_jyjl
									+ 'findPuerchase', option.header);
							// /p2p_action/account/login
						},

						chaxun : function($str) {

							return $http.post(option.url.api_jyjl + 'chaxun',
									$str, option.header);
							// /p2p_action/account/login
						},

						chaxunTongJi1 : function($str) {

							return $http.post(option.url.api_jyjl
									+ 'chaxunTongJi1', $str, option.header);
							// /p2p_action/account/login
						},
						chaxunTongJi2 : function($str) {

							return $http.post(option.url.api_jyjl
									+ 'chaxunTongJi2', $str, option.header);
							// /p2p_action/account/login
						}
					}
				})
